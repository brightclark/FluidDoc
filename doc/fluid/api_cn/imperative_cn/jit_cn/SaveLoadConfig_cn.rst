...
用于配置接口 :ref:`cn_api_imperative_jit_save` 和 :ref:`cn_api_imperative_jit_load` 存储载入 :ref:`cn_api_imperative_TranslatedLayer` 时的附加选项。
...
选择保存模型（ :ref:`cn_api_imperative_TranslatedLayer` ）的输出变量，通过指定的这些变量能够使模型仅计算特定的结果。
默认情况下，原始 :ref:`cn_api_fluid_dygraph_Layer` 的forward方法的所有返回变量都将配置为存储后模型 :ref:`cn_api_imperative_TranslatedLayer` 的输出变量。
